...
function collectProjectAgentsFiles(cwd: string): string[] {
  ...
  function walk(dir: string, depth = 0): void {
    ...
    for (const entry of entries) {
      if (entry.name === 'node_modules' || entry.name === '.git' || entry.name === 'dist' || entry.name === '.omc') continue;
...
export function getProjectContext(cwd: string): string {
  ...
  try {
    ...
    if (fs.existsSync(rootAgentsPath)) {
      const raw = fs.readFileSync(rootAgentsPath, 'utf8').trim();
      if (raw) {
        lines.push('');
        lines.push('Root AGENTS context:');
        lines.push(raw.split('\n').slice(0, 40).join('\n'));
      }
    }

    const agentFiles = collectProjectAgentsFiles(cwd)
      .map(filePath => path.relative(cwd, filePath))
      .filter(relative => relative !== 'AGENTS.md')
      .slice(0, 24);
    if (agentFiles.length > 0) {
      lines.push('');
      lines.push('Directory AGENTS files:');
      lines.push(...agentFiles.map(file => `- ${file}`));
    }
  } catch { /* ignore */ }
